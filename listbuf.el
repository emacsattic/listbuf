...
;; Copyright (C) 1991, 94, 95, 96, 97, 2002, 2006 Noah S. Friedman
...
(defun listbuf-get-Mode ()
  (cond ((stringp mode-name)
        ...
        ((fboundp 'format-mode-line)
         ;; Must specify the buffer in which to evaluate the terms in the
         ;; mode line string, otherwise it defaults to the selected
         ;; window's buffer, which usually will not be the correct one.
...
;; buffer name (ignoring case).  Uniqueness numbers on buffer names are
;; compared numerically.  See `listbuf-sorting-predicate'.
(defun listbuf-sort-by-buffer-name (v1 v2)
  (cond ((and (string-match "<[0-9]+>\\s-*\\'" (aref v1 2))
              (string-match "<[0-9]+>\\s-*\\'" (aref v2 2)))
         (let ((parts (mapcar (lambda (s)
                                (and (string-match "\\`\\(.*\\)<\\([0-9]+\\)>\\s-*\\'" s)
                                     (vector (downcase (match-string 1 s))
                                             (string-to-number (match-string 2 s)))))
                              (list (aref v1 2)
                                    (aref v2 2)))))
           (or (string-lessp (aref (nth 0 parts) 0) (aref (nth 1 parts) 0))
               (and (string-equal (aref (nth 0 parts) 0) (aref (nth 1 parts) 0))
                    (<            (aref (nth 0 parts) 1) (aref (nth 1 parts) 1))))))
        (t
...
(defun listbuf-sort-by-mode-and-buffer-name (v1 v2)
  (let ((mode1 (downcase (aref v1 6)))
	...
	(and (string-equal mode1 mode2)
             (listbuf-sort-by-buffer-name v1 v2)))))
